...
#define INSTANTIATE(F_TYPE)                                                                     \
  template class pdlp_restart_strategy_t<index_t, F_TYPE>;                                          \
                                                                                                \
  template __global__ void compute_distance_traveled_last_restart_kernel<index_t, F_TYPE>(          \
    const typename localized_duality_gap_container_t<index_t, F_TYPE>::view_t duality_gap_view,     \
    const F_TYPE* primal_weight,                                                                \
    F_TYPE* distance_traveled);                                                                 \
                                                                                                \
  template __global__ void pick_restart_candidate_kernel<index_t, F_TYPE>(                          \
    const typename localized_duality_gap_container_t<index_t, F_TYPE>::view_t avg_duality_gap_view, \
    const typename localized_duality_gap_container_t<index_t, F_TYPE>::view_t                       \
      current_duality_gap_view,                                                                 \
    typename pdlp_restart_strategy_t<index_t, F_TYPE>::view_t restart_strategy_view);               \
                                                                                                \
  template __global__ void adaptive_restart_triggered<index_t, F_TYPE>(                             \
    const typename localized_duality_gap_container_t<index_t, F_TYPE>::view_t                       \
  ...
  template __global__ void solve_bound_constrained_trust_region_kernel<int, F_TYPE, 128>(       \
    typename pdlp_restart_strategy_t<index_t, F_TYPE>::view_t restart_strategy_view,                \
    typename mip::problem_t<index_t, F_TYPE>::view_t op_problem_view,                               \
    ...
    const F_TYPE* target_radius);                                                               \
                                                                                                \
  template __global__ void target_threshold_determination_kernel<index_t, F_TYPE>(                  \
    typename pdlp_restart_strategy_t<index_t, F_TYPE>::view_t restart_strategy_view,                \
    const F_TYPE* target_radius,                                                                \
    const F_TYPE* max_primal_threshold,                                                         \
    const F_TYPE* max_dual_threshold);                                                          \
                                                                                                \
  template __global__ void compute_normalized_gaps_kernel<index_t, F_TYPE>(                         \
    typename localized_duality_gap_container_t<index_t, F_TYPE>::view_t avg_duality_gap_view,       \
    typename localized_duality_gap_container_t<index_t, F_TYPE>::view_t current_duality_gap_view);  \
                                                                                                \
  template __global__ void compute_new_primal_weight_kernel<index_t, F_TYPE>(                       \
    const typename localized_duality_gap_container_t<index_t, F_TYPE>::view_t duality_gap_view,     \
    ...
    F_TYPE* dual_step_size);                                                                    \
                                                                                                \
  template __global__ void compute_subgradient_kernel<index_t, F_TYPE>(                             \
    const typename pdlp_restart_strategy_t<index_t, F_TYPE>::view_t restart_strategy_view,          \
    const typename mip::problem_t<index_t, F_TYPE>::view_t op_problem_view,                         \
    const typename localized_duality_gap_container_t<index_t, F_TYPE>::view_t duality_gap_view,     \
