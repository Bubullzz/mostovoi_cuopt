  ...
  i_t nnz)
{
  ...
  for (i_t rank = 0; rank < nb_parts; rank++) {
    ...
    // with halo rows being empty (zero nnz). Without this padding the
    // per-shard cuSPARSE CSR descriptor would advertise n_rows = total_*
    // (because constraint-/objective-bounds setters in shard.cu fix
    // n_constraints / n_variables to TOTAL) while the offsets array would
    // only carry owned_*_size + 1 entries -- cusparseXcsrsort_bufferSizeExt
    // reads (n_rows + 1) entries via cs::copy_d2h and trips on
    // "invalid argument" before the first PDLP iteration.
