    ...
    all_coefficients{}
{
    ...
    for (int rank = 0; rank < nbDevice; rank++)
    {
        ...
        {
            // should only happen on last gpu
        ...
        std::transform(local_offsets.begin(), local_offsets.end(),
        local_offsets.begin(),
        ...
        std::copy(h_indices.begin() + start_row,
                   h_indices.begin() + end_row,
        local_indices.begin());

        std::vector<int> local_coeffs(nb_values);
        ...
        int cols = op_problem.n_variables;
    ...
    const i_t n_variables = op_problem.n_variables;
    for (int rank = 0; rank < nbDevice; rank++)
    {
        if (!is_test)
            cudaSetDevice(rank);
        ...
        assert(std::all_of(h_indices_rank.begin(), h_indices_rank.end(),
                          [n_variables](i_t j) {
                            return j >= 0 && j < n_variables;
                          }) &&
