...
__global__ void init_lhs_and_violation(typename fj_t<i_t, f_t>::climber_data_t::view_t fj)
{
  for (i_t cstr_idx = TH_ID_X; cstr_idx < fj.pb.n_constraints; cstr_idx += GRID_STRIDE) {
    ...
    auto delta_it = thrust::make_transform_iterator(
      thrust::make_counting_iterator(i_t(0)), [fj] __device__(i_t j) {
...
#define CUOPT_INSTANTIATE(F_TYPE)                                                                  \
  ...
  template __global__ void load_balancing_compute_workid_mappings<cuopt_int_t, F_TYPE>(            \
    typename fj_t<cuopt_int_t, F_TYPE>::climber_data_t::view_t fj,                                 \
    raft::device_span<cuopt_int_t> row_size_prefix_sum,                                            \
    raft::device_span<cuopt_int_t> var_indices,                                                    \
  ...
  compute_mtm_moves_kernel<cuopt_int_t, F_TYPE, MTMMoveType::FJ_MTM_VIOLATED, false>(              \
    typename fj_t<cuopt_int_t, F_TYPE>::climber_data_t::view_t fj, bool);                          \
  template __global__ void                                                                         \
  compute_mtm_moves_kernel<cuopt_int_t, F_TYPE, MTMMoveType::FJ_MTM_VIOLATED, true>(               \
    typename fj_t<cuopt_int_t, F_TYPE>::climber_data_t::view_t fj, bool);                          \
  template __global__ void                                                                         \
  compute_mtm_moves_kernel<cuopt_int_t, F_TYPE, MTMMoveType::FJ_MTM_SATISFIED, false>(             \
    typename fj_t<cuopt_int_t, F_TYPE>::climber_data_t::view_t fj, bool);                          \
  template __global__ void                                                                         \
  compute_mtm_moves_kernel<cuopt_int_t, F_TYPE, MTMMoveType::FJ_MTM_SATISFIED, true>(              \
    typename fj_t<cuopt_int_t, F_TYPE>::climber_data_t::view_t fj, bool);                          \
  ...
  get_launch_dims_compute_mtm_moves_kernel<cuopt_int_t,                                            \
                                           F_TYPE,                                                 \
                                           MTMMoveType::FJ_MTM_VIOLATED,                           \
                                           false>(int TPB, const raft::handle_t* handle_ptr);      \
  template std::pair<dim3, dim3>                                                                   \
  get_launch_dims_compute_mtm_moves_kernel<cuopt_int_t,                                            \
                                           F_TYPE,                                                 \
                                           MTMMoveType::FJ_MTM_VIOLATED,                           \
                                           true>(int TPB, const raft::handle_t* handle_ptr);       \
  ...
  get_launch_dims_load_balancing_mtm_compute_candidates<cuopt_int_t, F_TYPE>(                      \
    int TPB, const raft::handle_t* handle_ptr);                                                    \
  template std::pair<dim3, dim3>                                                                   \
  get_launch_dims_load_balancing_mtm_compute_scores<cuopt_int_t, F_TYPE>(                          \
    int TPB, const raft::handle_t* handle_ptr);                                                    \
  template std::pair<dim3, dim3>                                                                   \
  get_launch_dims_load_balancing_prepare_iteration<cuopt_int_t, F_TYPE>(                           \
    int TPB, const raft::handle_t* handle_ptr);                                                    \
  template void                                                                                    \
  launch_compute_mtm_moves_kernel<cuopt_int_t, F_TYPE, MTMMoveType::FJ_MTM_VIOLATED, false>(       \
    dim3 grid, dim3 blocks, void** kernel_args, rmm::cuda_stream_view stream);                     \
  template void                                                                                    \
  launch_compute_mtm_moves_kernel<cuopt_int_t, F_TYPE, MTMMoveType::FJ_MTM_VIOLATED, true>(        \
    dim3 grid, dim3 blocks, void** kernel_args, rmm::cuda_stream_view stream);                     \
  ...
  template void launch_handle_local_minimum_kernel<cuopt_int_t, F_TYPE>(                           \
    dim3 grid, dim3 blocks, void** kernel_args, rmm::cuda_stream_view stream);                     \
  template std::pair<dim3, dim3>                                                                   \
  get_launch_dims_update_changed_constraints_kernel<cuopt_int_t, F_TYPE>(                          \
    int TPB, const raft::handle_t* handle_ptr);                                                    \
